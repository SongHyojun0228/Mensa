...
<p>- 클라이언트(`GameClient`)는 `Socket`으로 서버에 연결하여 플레이어 입력(이동, 공격, 스킬 등)을 전송합니다.</p>
<p>- 서버(`GameServer`)는 모든 클라이언트로부터 입력을 받아 게임 로직을 처리하고, 모든 클라이언트에게 최신 게임 상태(위치, 몬스터, 총알, 점수 등)를 브로드캐스트합니다.</p>
<p>- 클라이언트는 해당 정보를 기반으로 게임 화면을 갱신합니다.</p>

<br>
<br>

<h2>👨🏻‍💻 기여자</h2>
https://github.com/SongHyojun0228
