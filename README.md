...
<p>
  이 프로젝트는 단순한 게임 구현을 넘어, 실시간 서버-클라이언트 구조와 UI 렌더링, 사용자 입력 처리,
  충돌 판정, 애니메이션 처리 등 <strong>게임 개발 전반에 필요한 핵심 로직</strong>을
  직접 설계하고 구현한 경험을 담고 있습니다.
</p>
...
<ul>
  <li>클라이언트 <code>GameClient</code>는 <code>Socket</code>으로 서버에 연결하여 플레이어 입력(이동, 공격, 스킬 등)을 전송</li>
  <li>서버 <code>GameServer</code>는 모든 클라이언트로부터 입력을 받아 게임 로직을 처리</li>
  <li>모든 클라이언트에게 최신 게임 상태(위치, 몬스터, 총알, 점수 등)를 브로드캐스트</li>
  <li>클라이언트는 해당 정보를 기반으로 게임 화면을 갱신</li>
</ul>
