...
// 스킬 이펙트 이미지 로드 (Explosion_1.png ~ Explosion_10.png)
            for (int i = 1; i <= 10; i++) {
                try {
                    BufferedImage img = ImageIO.read(getClass().getResource("/images/Explosion_"
                            + i + ".png"));
                    frames.add(img);
                } catch (IOException e) {
                    e.printStackTrace();
...
// 슬라임 애니메이션 프레임 업데이트
    ...
    public GamePanel() {
        ...
        try {
            ...
            runImage
                    = ImageIO.read(getClass().getResource("/images/Pink_Monster_Run_6.png"));
            idleImage = ImageIO.read(getClass().getResource("/images/Pink_Monster.png"));
            enemyImage
                    = ImageIO.read(getClass().getResource("/images/Slime1_Walk_full[1].png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
...
// 캐릭터 이미지 그리기 (좌우 반전 처리)
        ...
        Image displayedImage = facingRight ? currentFrameImage
                : flipImage((BufferedImage) currentFrameImage);
        g2d.drawImage(displayedImage, characterX, characterY, CHARACTER_WIDTH,
...
// 슬라임(Enemy) 그리기
        for (Enemy enemy : enemies) {
            g.drawImage(getEnemyFrame(enemy.frame), enemy.x, enemy.y, ENEMY_SIZE,
                    ENEMY_SIZE, this);
        }

// 총알 그리기
        for (Bullet bullet : bullets) {
            g.drawImage(bullet.image, bullet.x, bullet.y, BULLET_SIZE, BULLET_SIZE,
                    this);
        }
...
// 플레이어 체력바 그리기
    ...
    private void drawMPBar(Graphics g) {
        int barWidth = CHARACTER_WIDTH;
        int barHeight = 4;
        // 체력바 아래에 마나바를 위치시키기 위해 y 좌표 조정 (예: 체력바와 약간의 간격)
        int x = characterX;
        int y = characterY - 5 - 10 + 10;
        g.setColor(Color.BLACK);
        g.drawRect(x, y, barWidth, barHeight);
...
// 0.5초마다 플레이어 중심 기준 타격 범위 내 슬라임 공격으로 체력 감소 적용
    private void applyDamage() {
        ...
        int damage = effectiveCount * (effectiveCount > 0
                ? enemies.get(0).attackPower : 0);
        if (damage > 0) {
            playerHealth = Math.max(0, playerHealth - damage);
        }
        System.out.println("Player health: " + playerHealth + " (Damage: " + damage
                + " from " + count + " 슬라임)");
        repaint();
    }
